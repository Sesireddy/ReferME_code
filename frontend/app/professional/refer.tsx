...
import { View, StyleSheet, TouchableOpacity, Linking, Share } from "react-native";
...
function buildShareMessage(link: string) {
  return [
    "Join ReferME and connect job seekers with real opportunities.",
    "",
    "Post jobs, conduct mock interviews, earn credits, and build your professional network.",
    "",
...
export default function ProReferAFriend() {
  ...
  async function copy(text: string, label = "Copied!") {
    try { await Clipboard.setStringAsync(text); successAlert.show({ title: label, message: "Paste it anywhere you like.", intent: "success" }); } catch {}
  }
  async function nativeShare(message: string) {
    try { await Share.share({ message }, { dialogTitle: "Invite a friend to ReferME" }); } catch {}
  }
  function openSocialShare(channel: string) {
    ...
    let url = "";
    switch (channel) {
      case "whatsapp": url = `whatsapp://send?text=${enc}`; break;
      case "telegram": url = `https://t.me/share/url?url=${link}&text=${enc}`; break;
      case "facebook": url = `https://www.facebook.com/sharer/sharer.php?u=${link}&quote=${enc}`; break;
      case "instagram": copy(msg, "Copied for Instagram"); Linking.openURL("instagram://").catch(() => Linking.openURL("https://www.instagram.com/")); return;
      case "gmail": url = `mailto:?subject=${encodeURIComponent("Join me on ReferME")}&body=${enc}`; break;
      default: return nativeShare(msg);
    }
    Linking.openURL(url).catch(async () => { await nativeShare(msg); });
  }
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={["top"]}>
      ...
      <Screen refreshing={refreshing} onRefresh={load}>
        <LinearGradient colors={["#7C3AED", "#22C55E"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.hero}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
            <Ionicons name="gift" size={26} color="#fff" />
            <Txt style={styles.heroTitle}>Earn 25 Credits per signup</Txt>
          </View>
          <Txt style={styles.heroSub}>Refer your friends, relatives, and colleagues to ReferME and earn 25 Credits for every successful referral.</Txt>
        </LinearGradient>

        <Card style={{ marginTop: 16 }}>
          <Txt variant="label" style={{ color: colors.textSecondary }}>YOUR REFERRAL CODE</Txt>
          <View style={styles.codeRow}>
            <Txt style={styles.codeText} testID="ref-code" selectable>{info?.code || "—"}</Txt>
            <TouchableOpacity testID="copy-code" onPress={() => info && copy(info.code, "Code copied!")} style={styles.copyChip}>
          ...
          <View style={styles.linkRow}>
            ...
            <TouchableOpacity testID="copy-link" onPress={() => info && copy(info.link, "Link copied!")} style={styles.copyBtn}>
        ...
        <View style={styles.shareGrid}>
          ...
          <ShareTile testID="share-native" label="More" icon="share-social" color="#7C3AED" onPress={() => info && nativeShare(buildShareMessage(info.link))} />
        </View>
              ...
              <Card key={r.id} padding={12}>
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  ...
                  <View style={{ flex: 1, marginLeft: 10 }}>
                    ...
                    <Txt variant="small" style={{ color: colors.textSecondary }} numberOfLines={1}>{r.email_masked} · {new Date(r.created_at).toLocaleDateString()}</Txt>
                  </View>
                  <View style={[styles.statusPill, { backgroundColor: badgeBg(r.status) }]}>
                    <Txt style={{ color: badgeFg(r.status), fontWeight: "700", fontSize: 11, textTransform: "capitalize" }}>{r.status}</Txt>
                  </View>
                </View>
              </Card>
            ))}
...
const styles = StyleSheet.create({
  ...
  codeText: { fontSize: 22, fontWeight: "900", letterSpacing: 1.5, color: "#7C3AED", flex: 1 },
  copyChip: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 10, borderWidth: 1, borderColor: "#7C3AED55" },
  copyChipText: { color: "#7C3AED", fontWeight: "700", fontSize: 12 },
  ...
  copyBtn: { width: 34, height: 34, borderRadius: 8, backgroundColor: "#7C3AED", alignItems: "center", justifyContent: "center" },
